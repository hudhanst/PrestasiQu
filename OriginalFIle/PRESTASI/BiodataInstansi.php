        ...
        <center>
          <table id="myTable">
            <tr>
              ...
              <th onclick="SortStringData(2)">Nama Instansi</th>
              <th onclick="SortStringData(3)">Jenis Instansi</th>              
              <th>VIEW</th>
              <th>EDIT</th>
              <th>DELETE</th>
            </tr>
            <?php
$fetchsql = "SELECT * FROM prestasi_instansi";
$fetchresault = $conn->query($fetchsql);
$no = 1;
if($fetchresault->num_rows>0){
  while($row = $fetchresault->fetch_assoc()) {
    echo "<tr>";
    echo "<td hidden >".$row["iInstansiId"]."</td>";
    echo "<td>".$no++."</td>";
    echo "<td>".$row["vcNamaInstansi"]."</td>";
    echo "<td>".$row["vcJenisInstansi"]."</td>";
    echo "<td><input type='button' name='viewe' value='view' id=".$row['iInstansiId']." class='viewe_data' onclick='model3()'/></td>";  
    echo "<td><input type='button' name='updatee' value='update' id=".$row['iInstansiId']." class='updatee_data' onclick='model1()'/></td>";  
    echo "<td><input type='button' name='deletee' value='delete' id=".$row['iInstansiId']." class='deletee_data' onclick='model2()'/></td>";  
    echo "</tr>";
      ...
      <div class="modal-content">
        ...
        <form method="POST" action="PHP/proses.php">
          <ul>
            <li>
              Nama Instansi<br>
               <input type="text" id="" name="TBNamaInstansi" placeholder="Masukkan Nama Instansi:"
                required>
            </li>
            <li>
              Jenis Instansi<br>
              <select name="TBJenisInstansi" required>
                <option value=""></option>
            ...
            <li>
              Keterangan<br>
               <input type="text" id="" name="TBKeteranganINstansi" placeholder="Masukkan Keterangan:" >
            </li>
          </ul>
          <input type="submit" name="FormTambahInstansi">
        </form>
      </div>
    </div>
  </div>

  <div class="EditInstansiModal">
    <!-- The Modal -->
    <div id="myModal1" class="modal">
      ...
      <div class="modal-content">
        ...
        <div id="EditInstansiModal"></div>
      </div>
    </div>
  </div>

  <div class="HapusInstansiModal">
    <!-- The Modal -->
    <div id="myModal2" class="modal">
      ...
      <div class="modal-content">
        ...
        <h1>Hapus Instansi</h1>
        <h2>Apakah Anda Yakin Menghapus Data Tersebut?</h2>

        <div id="DeleteInstansiModal"></div>
      </div>
    </div>
  </div>

  <div class="VieweInstansiModal">
    <!-- The Modal -->
    <div id="myModal3" class="modal">
      ...
      <div class="modal-content">
        ...
        <h1>View Data Instansi</h1>
        

        <div id="VieweInstansiModal"></div>
      </div>
    </div>
  </div>
...
<script>
  $(document).ready(function () {
    $('.updatee_data').click(function () {
      var UpdateInstansiId = $(this).attr("id");
      $.ajax({
        url: "PHP/modals.php",
        method: "post",
        data: {
          UpdateInstansiId: UpdateInstansiId
        },
        success: function (data) {
          $('#EditInstansiModal').html(data);
  ...
  $(document).ready(function () {
    $('.deletee_data').click(function () {
      var DeleteInstansiId = $(this).attr("id");
      $.ajax({
        url: "PHP/modals.php",
        method: "post",
        data: {
          DeleteInstansiId: DeleteInstansiId
        },
        success: function (data) {
          $('#DeleteInstansiModal').html(data);
          //  $('#dataModal').modal("show");  
        }
      });
    });
  ...
  $(document).ready(function () {
    $('.viewe_data').click(function () {
      var VieweInstansiId = $(this).attr("id");
      $.ajax({
        url: "PHP/modals.php",
        method: "post",
        data: {
          VieweInstansiId: VieweInstansiId
        },
        success: function (data) {
          $('#VieweInstansiModal').html(data);
          //  $('#dataModal').modal("show");  
        }
      });
    });
